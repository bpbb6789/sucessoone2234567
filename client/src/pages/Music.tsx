...
export default function Music() {
  ...
  if (viewState.view === 'nowplaying' && player.currentTrack) {
    return (
      <div className="min-h-screen bg-gradient-to-b from-gray-900 to-black text-white flex flex-col items-center justify-center p-8" data-testid="now-playing-view">
        <button 
          onClick={goBack}
        ...
        <div className="w-80 h-80 bg-gradient-to-br from-green-900 to-black rounded-lg mb-8 flex items-center justify-center">
        ...
        {/* Track Info */}
        <h1 className="text-4xl font-bold mb-2 text-center">{player.currentTrack.title}</h1>
        <p className="text-xl text-gray-400 mb-8">{player.currentTrack.artist}</p>
        ...
        <div className="p-6">
          ...
          <div className="flex items-start space-x-6 mb-8">
            ...
            <div className="flex-1">
              <h1 className="text-5xl font-bold mb-4">{viewState.selectedAlbum.title}</h1>
              <p className="text-xl text-gray-300 mb-2">{viewState.selectedAlbum.artist}</p>
              ...
              <div className="flex items-center space-x-4">
                <button className="bg-green-500 text-black px-8 py-3 rounded-full font-semibold hover:bg-green-400 flex items-center space-x-2">
                  <Play className="w-5 h-5" fill="currentColor" />
                  <span>Play</span>
                </button>
                <button className="border border-gray-600 px-6 py-3 rounded-full hover:bg-gray-800 flex items-center space-x-2">
                  <Shuffle className="w-5 h-5" />
                  <span>Shuffle</span>
                </button>
                <Heart className="w-8 h-8 text-gray-400 hover:text-white cursor-pointer" />
                <Share className="w-8 h-8 text-gray-400 hover:text-white cursor-pointer" />
                <MoreHorizontal className="w-8 h-8 text-gray-400 hover:text-white cursor-pointer" />
              </div>
            </div>
          </div>
            ...
            {isLoading ? (
              <div className="space-y-2">
                {Array.from({ length: 8 }).map((_, i) => (
                  <div key={i} className="animate-pulse grid grid-cols-12 gap-4 p-4">
              ...
              <div className="space-y-1">
                {albumTracks.map((track, index) => (
                  <div
                    key={track.id}
                    className="grid grid-cols-12 gap-4 p-4 rounded-lg hover:bg-white/10 group cursor-pointer"
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-purple-900/20 to-black text-white" data-testid="page-music">
      ...
      <div className="p-6">
                ...
                >
                  <div className="relative mb-3">
                    ...
                    <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity rounded-lg flex items-center justify-center">
                      <Play className="w-12 h-12 text-green-500" fill="currentColor" />
                    </div>
                  </div>
        ...
        <div className="mb-8">
          <h2 className="text-2xl font-bold mb-4">More of what you like</h2>
          {isLoading ? (
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4">
              {Array.from({ length: 6 }).map((_, i) => (
                <div key={i} className="animate-pulse">
