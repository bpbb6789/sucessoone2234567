  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-purple-900/20 to-black text-white" data-testid="page-music">
      ...
      <div className="p-6">
        ...
        <h1 className="text-3xl font-bold mb-2">Fresh new music</h1>
      </div>

      {/* Fresh New Music Grid */}
      <div className="px-6 mb-8">
        {isLoading ? (
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
            {Array.from({ length: 8 }).map((_, i) => (
              <div key={i} className="animate-pulse" data-testid={`fresh-music-skeleton-${i}`}>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
            {freshTracks.map((track) => (
              <div
                ...
                data-testid={`fresh-track-${track.id}`}
              >
                ...
                <h3 className="text-sm font-medium mb-1 truncate" data-testid={`track-title-${track.id}`}>
                  {track.title}
                </h3>
                <p className="text-xs text-gray-400 truncate" data-testid={`track-artist-${track.id}`}>
                  {track.artist}
                </p>
              </div>
            ))}
        ...
        {isLoading ? (
          <div className="space-y-3">
            {Array.from({ length: 6 }).map((_, i) => (
              <div key={i} className="flex items-center space-x-4 animate-pulse" data-testid={`hits-skeleton-${i}`}>
          ...
          <div className="space-y-2">
            {todaysHits.map((track, index) => (
              <div
                ...
                onClick={() => playTrack(track)}
                data-testid={`hit-track-${track.id}`}
              >
                ...
                <div className="flex-1 min-w-0">
                  <h3 className="text-sm font-medium truncate" data-testid={`hit-title-${track.id}`}>
                    {track.title}
                  </h3>
                  <p className="text-xs text-gray-400 truncate" data-testid={`hit-artist-${track.id}`}>
                    {track.artist}
                  </p>
                </div>
                <div className="flex items-center space-x-2">
                  <Heart className="w-4 h-4 text-gray-400 hover:text-green-500 cursor-pointer" />
                  <span className="text-xs text-gray-400" data-testid={`hit-duration-${track.id}`}>
